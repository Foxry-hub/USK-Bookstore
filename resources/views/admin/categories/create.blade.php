    ...
    <section class="mx-auto max-w-xl rounded-2xl border border-slate-200 bg-white p-6">
        ...
        <form action="{{ route('admin.categories.store') }}" method="POST" class="mt-5 space-y-4">
            ...
            <div>
                ...
                <input type="text" name="name" value="{{ old('name') }}" class="w-full rounded-xl border border-slate-300 px-4 py-3 text-sm" required>
                @error('name') <p class="mt-1 text-xs text-rose-600">{{ $message }}</p> @enderror
            </div>

            <div class="flex gap-3">
                <button type="submit" class="rounded-xl bg-brand-500 px-4 py-3 text-sm font-semibold text-white">Simpan</button>
