        ...
        <div class="mt-6 grid gap-4 lg:grid-cols-2 xl:grid-cols-3">
            @forelse ($orders as $order)
                @php
                    ...
                    $isCashOrder = strtoupper((string) $order->payment_method) === 'CASH';
                    $displayStatus = $order->status;

                    if ($isCashOrder && $order->cash_payment_confirmed_at) {
                        $displayStatus = 'Selesai';
                    }

                    $displayStatusClass = $statusClasses[$displayStatus] ?? 'bg-slate-100 text-slate-700';
                @endphp
                <article class="flex h-full flex-col rounded-2xl border border-slate-200 bg-white p-5 shadow-sm transition hover:border-slate-300 hover:shadow-md">
                    <div class="flex items-start justify-between gap-3">
                        <div class="min-w-0">
                            <h2 class="break-words text-2xl font-extrabold leading-[1.2] tracking-tight text-slate-900">{{ $order->order_code }}</h2>
                            <p class="mt-1 text-xs text-slate-500">{{ $order->created_at->format('d M Y, H:i') }}</p>
                        </div>
                        <span class="shrink-0 rounded-full px-3 py-1.5 text-xs font-semibold {{ $displayStatusClass }}">{{ $displayStatus }}</span>
                    </div>

                    <div class="mt-4 rounded-xl border border-slate-200 bg-slate-50 p-4">
                        <p class="text-lg font-bold leading-tight text-slate-900">{{ $order->user->name }}</p>
                        <p class="mt-1 text-sm text-slate-600">{{ $order->user->email }}</p>
                        <p class="mt-3 text-base font-bold text-slate-900">Total: Rp {{ number_format($order->total_price, 0, ',', '.') }}</p>

                        <div class="mt-3 text-sm text-slate-700">
                            @if ($order->payment_method === 'COD')
                                <span class="font-semibold">Status Pembayaran:</span>
                                <span class="ml-2 inline-flex rounded-full bg-slate-200 px-2.5 py-0.5 text-xs font-semibold text-slate-700">COD</span>
                            @elseif ($isCashOrder)
                                <span class="font-semibold">Status Pembayaran:</span>
                                @if ($order->status === 'Menunggu Pembayaran')
                                    <span class="ml-2 inline-flex rounded-full bg-amber-100 px-2.5 py-0.5 text-xs font-semibold text-amber-800">Menunggu Pembayaran Tunai</span>
                                @elseif ($order->cash_payment_confirmed_at)
                                    <span class="ml-2 inline-flex rounded-full bg-emerald-100 px-2.5 py-0.5 text-xs font-semibold text-emerald-800">Pembayaran Tunai Dikonfirmasi</span>
                                @endif
                            @elseif ($order->midtrans_transaction_status)
                                <span class="font-semibold">Status Pembayaran:</span>
                                <span class="ml-2 inline-flex rounded-full px-2.5 py-0.5 text-xs font-semibold {{ $paymentStatusClasses[$order->midtrans_transaction_status] ?? 'bg-slate-100 text-slate-700' }}">{{ $order->midtrans_transaction_status }}</span>
                            @endif
                        </div>

                        @if ($isCashOrder && $order->cash_payment_confirmed_at)
                            <div class="mt-3 rounded-lg border border-slate-200 bg-white p-3 text-sm leading-tight text-slate-700">
                                <p><span class="font-semibold text-slate-900">Uang Diterima:</span> Rp {{ number_format($order->cash_amount_paid, 0, ',', '.') }}</p>
                                <p class="mt-1"><span class="font-semibold text-slate-900">Kembalian:</span> Rp {{ number_format($order->cash_change, 0, ',', '.') }}</p>
                            </div>
                        @endif
                    </div>

                    @if (! $isCashOrder)
                        <div class="mt-3 grid grid-cols-3 gap-2 rounded-xl border border-slate-200 bg-slate-50 p-3 text-xs text-slate-600">
                            <p class="leading-tight">Shipped:<br><span class="font-semibold text-slate-900">{{ $order->shipped_at?->format('d M Y, H:i') ?? '-' }}</span></p>
                            <p class="leading-tight">Estimasi:<br><span class="font-semibold text-slate-900">{{ $order->estimated_delivery_at?->format('d M Y, H:i') ?? '-' }}</span></p>
                            <p class="leading-tight">Selesai:<br><span class="font-semibold text-slate-900">{{ $order->received_at?->format('d M Y, H:i') ?? '-' }}</span></p>
                        </div>
                    @endif
                    ...
                    <div class="mt-3 rounded-xl border border-slate-200 bg-slate-50 p-3">
                        <div class="space-y-2">
                            @foreach ($order->items as $item)
                                <div class="flex items-start justify-between gap-2 text-xs text-slate-700">
                                    <span class="line-clamp-2">{{ $item->book->title }} x {{ $item->quantity }}</span>
                                    <span class="shrink-0 font-semibold">Rp {{ number_format($item->subtotal, 0, ',', '.') }}</span>
                                </div>
                            @endforeach
                        </div>
                    </div>
                    ...
                    <div class="mt-auto flex flex-wrap items-center gap-2 border-t border-slate-200 pt-4">
                        ...
                        @if (! $isCashOrder && $order->status === 'Diproses')
                            <form action="{{ route('admin.orders.update-status', $order) }}" method="POST">
                                @csrf
                                @method('PATCH')
                        ...
                        @if (! $isCashOrder && $order->status === 'Dikirim')
                            <form action="{{ route('admin.orders.update-status', $order) }}" method="POST">
                                @csrf
                                @method('PATCH')
            ...
            @empty
                <div class="rounded-2xl border border-dashed border-slate-300 bg-white px-4 py-10 text-center text-sm text-slate-500 lg:col-span-2 xl:col-span-3">
